...
from core.models import Assessment, Class, OnlineQuestion, Term
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.allow_multiple_classes:
            self.fields['target_classes'] = forms.ModelMultipleChoiceField(
                ...
                help_text=(
                    'Creates one assessment copy per selected class so existing approval, submission, and scoring flows stay single-class.'
                    if self.hide_class_assigned_when_multi else
                    'Optional. Creates additional assessment copies for the selected classes using the current edited content.'
                ),
            )
